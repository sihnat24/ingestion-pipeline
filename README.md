...
This starts InfluxDB (`:8086`), Grafana (`:3000`), and the bot. The bot automatically restarts if it goes down (`restart: unless-stopped`).

**3. Run locally (without Docker)**
```bash
pip install -r requirements.txt
python state/ingest_15m_btc.py
```

Requires a running InfluxDB instance and environment variables set accordingly.
...
Log in with the `INFLUX_USERNAME` and `INFLUX_PASSWORD` values from your `.env` file. From there you can query any of the InfluxDB measurements directly or view existing dashboards tracking orderbook delta ingestion rate and BTC spot price.
...
```bash
python state/btc_terminal_plots.py
```
